...
GO
/****** Object:  StoredProcedure [dbo].[CMI_MasterData_Category_PrepareSyncList]    Script Date: 8/15/2018 11:22:56 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[CMI_MasterData_Category_PrepareSyncList]
AS
BEGIN
	SET NOCOUNT ON;
	...
	DECLARE @CategoryList TABLE (
		...
		,CategoryID VARCHAR(100),LastChgDateTime datetime,IsActive_Name varchar(5)
		)
	...
	DECLARE @LOOP_CategoryID VARCHAR(100)
...
BEGIN TRY
	...
	--Prepare category list from productcatalog and set number of categories to insert / update
	INSERT INTO @CategoryList (CategoryID,LastChgDateTime,IsActive_Name)
	SELECT top 5 Code,LastChgDateTime, IsActive_Name
	FROM  [PRODUCTCATALOGDBSERVER].[MDS2].[mdm].[viw_PC_Category] 
	WHERE (
	...
	BEGIN
		...
		SELECT @LOOP_CategoryID = CategoryID,@LOOP_IsActive_Name =IsActive_Name 
		FROM @CategoryList
		WHERE RowID = @CurrentRowID

		--set the Target ID
		SELECT @CurrentTargetEntityID = 'CT'+@LOOP_CategoryID 
		FROM [MasterEntityMap](NOLOCK)
		WHERE [SourceEntityID] = @LOOP_CategoryID
		AND [EntityTypeDDID] = 114
		...
		BEGIN
			...
			VALUES (
				...
				,'CT'+@LOOP_CategoryID ,getdate(),getdate()
				,0
				)
